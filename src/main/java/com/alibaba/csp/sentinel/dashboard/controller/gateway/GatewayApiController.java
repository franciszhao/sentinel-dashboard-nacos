...
public class GatewayApiController extends BaseSyncDataController<ApiDefinitionEntity> {
...
//            rules = repository.saveAll(rules);
            List<ApiDefinitionEntity> rules = this.list(ruleProvider, app);
...
//        List<ApiDefinitionEntity> allApis = repository.findAllByMachine(MachineInfo.of(app.trim(), ip.trim(), port));
        ...
        try {
            List<ApiDefinitionEntity> allApis = this.list(ruleProvider, app);
...
//        ApiDefinitionEntity entity = repository.findById(id);
        ApiDefinitionEntity entity = this.findById(ruleProvider, reqVo.getApp(), id);
        if (entity == null) {
...
//            publishRules(entity.getApp());
    ...
    public Result<Long> deleteApi(Long id, @RequestParam("app") String app) {
        if (id == null) {
            return Result.ofFail(-1, "id can't be null");
        }
...
//        ApiDefinitionEntity oldEntity = repository.findById(id);
        ApiDefinitionEntity oldEntity = this.findById(ruleProvider, app, id);
        if (oldEntity == null) {
...
//    }
    ...
    @Override
    protected void format(ApiDefinitionEntity entity, String app) {

    }

    @Override
    protected void merge(ApiDefinitionEntity entity, ApiDefinitionEntity oldEntity) {
